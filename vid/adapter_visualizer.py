...
class AdapterPatternVisualization(Scene):
    ...
    def visualize_protocol_adapter(self):
        ...
        # Code-Beispiel - Verwenden eines Textobjekts statt Code-Klasse
        code_text_content = """
        ...
        # Statt Code-Klasse verwenden wir ein Text-Objekt mit einem Hintergrund
        code_box = Rectangle(height=4, width=6.5, color=WHITE).set_fill(BLACK, opacity=0.8)
        code_box.to_edge(DOWN, buff=0.5)
        
        code_content = Text(code_text_content, font_size=16).move_to(code_box)
        code_group = VGroup(code_box, code_content)
        
        self.play(Create(code_box), Write(code_content))
        ...
        self.play(
            ...
            FadeOut(code_group), FadeOut(title)
        )
